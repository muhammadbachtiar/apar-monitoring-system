...
import {Button, Col, Form, Accordion, Row, Breadcrumb, Spinner} from 'react-bootstrap';
...
function AddInspection6Monthly() {
    ...
    const handleAddInspection = async (data: FieldValues) => {
        ...
        try {
          ...
          const response = await fetch(`${domainApi}/api/v1/6monthly-inspections`, {
            method: 'POST',
            body: formData,
            headers: {
    ...
    useEffect(() => {
        const fetchAparData = async () => {
          ...
          try {
            ...
            if (response.ok) {
              const data = await response.json();
              setAparData({apar_number: data.data.apar_number, apar_type: data.data.apar_type, location: data.data.location.location_name, last_6montly_check_time: data.data.last_6montly_check_time, last_filing_time: data.data.last_filing_time})
            } else {
          ...
          {loading ?
            ...
            <>
              <Breadcrumb>
                  ...
                  <Breadcrumb.Item linkAs={Link} linkProps={{ to: '/inspection-6-monthly' }}>Pemeriksaan APAR</Breadcrumb.Item>
              ...
              <div className="row my-3 align-items-center">
                  <div className="col">
                      ...
                      <Form className='shadow p-3 mb-2 bg-body-tertiary rounded' onSubmit={handleSubmit((data) => handleAddInspection(data))}>
                          ...
                          <Link to={"/inspection-6-monthly"}><Button className='btn-secondary'>Kembali</Button></Link>
...
export default AddInspection6Monthly;